...
function openedit(){
	layer.open({
		...
		shade: 0.8,
		area: ['650px', '70%'],
		content: contextPath + '/VersionController/addVersion'
	});  			
}
...
function history(dowtype){
	layer.open({
		...
		shade: 0.8,
		area: ['90%', '85%'],
		content: contextPath + '/VersionController/queryVersionHis?dowtype='+dowtype
	}); 
}

function checkDelversion(id,dowtype) {
		var url =contextPath + '/VersionController/checkDelVersionById';
		$.ajax({
			url : url,
			timeout : 300000,
			dataType : "json",
			...
			success : function(data) {
				if(data.status == "success"){
					...
					layer.alert("修改成功",{icon: 5,closeBtn: 0},function(index){
						setTimeout("location.reload()",100); //父页面刷新 必须在关闭iframe之前
						ShowLoading();
					});
				}
